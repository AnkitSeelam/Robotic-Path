...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected Path dijkstrasShortestPath(T start, T end) throws NoSuchElementException{
    ...
    PriorityQueue<Path> path = new PriorityQueue<Path>();

    LinkedList<Vertex> visitedVertex = new LinkedList<Vertex>();
    visitedVertex.add(vertices.get(start));
    path.add(new Path(vertices.get(start)));

    while (path.peek() != null) {
      if (path.peek().end.data.equals(end))
        return path.poll();
      Path dequeued = new Path(path.poll());
      LinkedList<Edge> edgesFound = new LinkedList<Edge>();
      for (int i = 0; i < dequeued.end.edgesLeaving.size(); i++) {
        edgesFound.add(i, dequeued.end.edgesLeaving.get(i));
      }
      for (int j = 0; j < dequeued.end.edgesLeaving.size(); j++) {
        Path newPath = new Path(dequeued);
        if (!visitedVertex.contains(edgesFound.peekFirst().target)) {
          visitedVertex.add(edgesFound.peekFirst().target);
          newPath.extend(edgesFound.pop());
          path.add(newPath);
        } else {
          edgesFound.pop();
        }
